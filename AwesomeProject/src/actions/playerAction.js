...
import { searchEntry } from '../fetch/search'
import { getMusicLrc, getMusicUrlById } from '../fetch/fetchAudio'

export function setPlayMusicList(playMusicList) {
  return {
...
export function setCurrentMusicInfo(musicInfo) {
  return (store) => {
    const { player = {} } = store || {}
...
export async function setSearchResultList(keywords) {
  const result = await searchEntry(keywords)
...
export async function playFindMusic(musicInfo) {
...
export function addSongToMusicList(musicInfo) {
  return (store) => {
    const { player = {} } = store || {}
    const { musicList = [] } = player
    let flag = true
...
export function playMusicList(id) {
  return async (store) => {
...
export function playMusicListNext() {
  return async (store) => {
    ...
    if(status.cyclicalPattern === 'repeat') {
    ...
    if(status.cyclicalPattern === 'shuffle') {
...
export function setCurrentMusicDuration(value) {
  return setPlayerStatus({
    duration: value,
  })
}

export function setMusicCurrentTime(value) {
  return setPlayerStatus({
    currentTime: value,
  })
}

export async function setCurrentMusicLrc(id) {
